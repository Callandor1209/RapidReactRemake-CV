...
public class DrivetrainDefaultCommand extends Command {
  ...
  private double applyDeadband(double x) {
    if (Math.abs(x) > 0.0144) {
      ...
      double power =0.07/Robot.DRIVETRAIN_SUBSYSTEM.getMaxVelocity() * sign + (1 - 0.07/Robot.DRIVETRAIN_SUBSYSTEM.getMaxVelocity()) * percentage * sign;
      return power;
    } else {
      return 0;
    }
  ...
  private double applyRotationalDeadband(double x) {
    if (Math.abs(x) > 0.0144) {
      double percentage = (Math.abs(x) - 0.0144) / (1 - 0.0144);
      double sign = Math.copySign(1, x);
      double power = 0.01/Robot.DRIVETRAIN_SUBSYSTEM.getMaxRotationalVelocity()* sign + (1 - 0.01/Robot.DRIVETRAIN_SUBSYSTEM.getMaxRotationalVelocity()) * percentage * sign;
      return power;
    } else {
      return 0;
    }

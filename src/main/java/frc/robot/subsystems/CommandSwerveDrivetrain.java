...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
    ...
    public void periodic() {
        ...
        Logger.recordOutput("Swerve Module States", this.getState().ModuleStates);
        Logger.recordOutput("Swerve Chassis Speeds", this.getState().Speeds);
    ...
    public void drive(double x, double y, double r) {
        setControl(Robot.SWERVE_REQUEST_DRIVE.withVelocityX(x)
                .withVelocityY(y)
    ...
    public Pose2d getPoseMA() {
        // return Robot.POSE_ESTIMATOR_6328.getPose();
        return getWPIEstimatorPose();
    }
      @AutoLogOutput
    public Pose2d getWPIEstimatorPose() {
    ...
    public Rotation2d getGyroscopeRotation() {
       ...
       public Pose2d getTargetPoseFromRobotRelativeChange(double xChange, double yChange) {
    
        Transform2d transform = new Transform2d(xChange, yChange, new Rotation2d());

        return getPose().plus(transform);
    }

        public double[] calculateSpeeds(Pose2d target, PIDController pidController) {
        double xSpeed  = pidController.calculate(getPose().getX(), target.getX());
        double ySpeed  = pidController.calculate(getPose().getY(), target.getY());
        return new double[] {xSpeed, ySpeed};
    }

        public void driveAutoWithFeedforward(ChassisSpeeds robotRelativeSpeeds, double feedforwards) {
       setControl(
                    Robot.PATH_APPLY_ROBOT_SPEEDS.withSpeeds(robotRelativeSpeeds)
